...
void ActionExecution::finished()
{
  ...
  if (status_update_callback_)
    status_update_callback_(types::ActionStatus::FINISHED, std::nullopt);
...
//=============================================================================
void ActionExecution::failed(const std::string& result_description)
{
  Execution::failed(result_description);
}
